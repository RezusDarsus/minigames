...
const homeMarkup = (): string => `
  <div class="site-shell">
    ...
    <main id="home">
      ...
      <section class="section new-games" aria-labelledby="new-games-title"><div class="section-heading section-heading--with-actions"><h2 id="new-games-title">New Games</h2><div class="carousel-actions"><button type="button" tabindex="-1" aria-disabled="true" aria-label="Previous games">${arrowBackIcon}</button><button class="is-primary" type="button" tabindex="-1" aria-disabled="true" aria-label="Next games">${arrowForwardIcon}</button></div></div><div class="game-carousel">${gameCards.map((game) => cardMarkup(game)).join('')}</div></section>
